...
void test_playing_never_sleeps() {
  sleep::SleepManager mgr;
  uint32_t now = 1000;
  for (int i = 0; i < 100; ++i, now += kPoll) {
    sleep::Decision d = mgr.updateAndDecide(now, true, false);
...
void test_short_inactivity_stays_active() {
  ...
  now += 5 * 60 * 1000;
  sleep::Decision d = mgr.updateAndDecide(now, false, false);
  TEST_ASSERT_FALSE(d.should_sleep);
}
...
void test_long_inactivity_sleeps() {
  ...
  now += sleep::kInactivityThresholdMs;
  sleep::Decision d = mgr.updateAndDecide(now, false, false);
  TEST_ASSERT_TRUE(d.should_sleep);
  TEST_ASSERT_EQUAL_UINT32(sleep::kSleepIntervalMs, d.duration_ms);
}
...
void test_sleep_timer_expires_wakes_up() {
  ...
  mgr.updateAndDecide(now, false, false);
  now += sleep::kInactivityThresholdMs;
  sleep::Decision d1 = mgr.updateAndDecide(now, false, false);
  TEST_ASSERT_TRUE(d1.should_sleep);

  // Attendre 60 s (durée du sommeil).
  now += sleep::kSleepIntervalMs;
  sleep::Decision d2 = mgr.updateAndDecide(now, false, false);
...
void test_button_during_sleep_wakes_immediately() {
  ...
  now += sleep::kSleepIntervalMs / 2;

  // Appui de bouton.
  sleep::Decision d = mgr.updateAndDecide(now, false, true);
