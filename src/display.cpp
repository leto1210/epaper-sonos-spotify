...
void showBootScreen(const char* status) {
  ...
  epaper.drawString("ePaper Spotify", kMargin + 30, kMargin + 40, 6);

  epaper.setTextColor(TFT_BLUE);
  epaper.drawString("Sonos sur reTerminal E1002", kMargin + 30, kMargin + 110, 4);

  epaper.setTextColor(TFT_BLACK);
  epaper.drawString(status, kMargin + 30, epaper.height() / 2 + 20, 4);

  epaper.setTextColor(TFT_RED);
  epaper.drawString(epaper_spotify::kFirmwareVersion, kMargin + 30,
                    epaper.height() - kMargin - 50, 4);
